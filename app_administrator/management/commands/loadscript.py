...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument(
            "with_clear",
            type=str,
            help="Delete only migrations files. " 'Set "no_clear" for cancel deleting',
        )
        parser.add_argument(
            "--db",
            action="store_true",
    ...
    def handle(self, *args, **kwargs) -> None:
        ...
        if not fixtures_list:
            self.stdout.write(
                self.style.WARNING(f'The folder "{FOLDER_FIXTURES}" is empty.')
            )
        else:
            while fixtures_list:
                err_list.clear()
                for item in fixtures_list:
                    ...
                    try:
                        management.call_command(
                            "loaddata",
                            os.path.normpath(os.path.join(FOLDER_FIXTURES, item)),
                        )
                        ...
                        self.stdout.write(
                            self.style.WARNING(f"{err} when loading {item}")
                        )
            ...
            if err_list:
                self.stdout.write(
                    self.style.WARNING("Not all fixtures have been loaded. Check it:")
                )
    ...
    def _remove_database(self) -> None:
        ...
        if os.path.exists(path_db):
            ...
            self.stdout.write(
                self.style.WARNING(f"Database file {path_db} does not exist.\n")
            )
